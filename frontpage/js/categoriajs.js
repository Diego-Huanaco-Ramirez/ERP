  ...
  var bandera = false;
     if(tipo_cat == 1) 
                ;  
                 ...
                 arraycat =    arraycatsbsb ;
     var strInput = $(obj).val();
 console.log('busqueda'+strInput+'cant:'+arraycat.length+';tipo:'+tipo_cat);
    //
    var i;
                  ...
                  if(tipo_cat == 1) 
                    ...
                    // alert(tipo_cat);
                     cantcategoria(myObj.length,tipo_cat,value)
                         llenarselect(myObj,objdestino,'cat');
              }else{
  ...
  //   console.log(id);
    if(tipo_cat == 1 ){   
      ...
      //  $("#PROD_CODIGO"+value).val('');
        ...
        $("#LBL_CAT_ID"+value).text(cat_id1);
          $("#LBL_CAT_IDB"+value).text(cat_id1);
        ...
        $("#PROD_CAT_ID"+value).val(cat1 +cat2);
         $("#LBL_CAT_ID"+value).text(cat_id2);
         $("#LBL_CAT_IDB"+value).text(cat_id2);
    } else if(tipo_cat == 3  ){
        bandera = true;
        cat_id3 = id;
        cat3 = obj.options[obj.selectedIndex].getAttribute('valor');
        $("#PROD_CAT_ID"+value).val(cat1+cat2+cat3);
        $("#LBL_CAT_ID"+value).text(cat_id3);
          $("#LBL_CAT_IDB"+value).text(cat_id3);
    }
 }else 
     ;// $("#PROD_CODIGO"+value).val('');
    if( bandera  ){
...
function llenarselect(myObj,objdestino,prefijo){
    deshabilitacat(objdestino,0);
    var arr = myObj;
    if(objdestino !== null){
    ...
    for(i=0; i< arr.length; i++){
        var c = document.createElement("option");
        c.text = arr[i][prefijo+"_nombre"];
        c.value = arr[i][prefijo+"_id"];
        c.valor = arr[i][prefijo+"_etiqueta"];
        c.setAttribute('valor',arr[i][prefijo+"_etiqueta"]);
    //    x.options.add(c, parseInt(arr[i][prefijo+"_id"]));
        x.options.add(c,pos); //posición en el objeto
        pos++;
    }
  ...
  function deshabilitacat(obj, opc){
       if(opc==1){
  ...
  function limpiacat(value){
    ...
    deshabilitacat(nombre_scat,1);
    deshabilitacat(nombre_sscat,1)

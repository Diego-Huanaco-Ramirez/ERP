...
function listarproductos(){
     ...
     //    $option=7;
         ...
         if($option == 2){
             ...
             if(isset($_GET['cat_id']) && intval($_GET['cat_id'])>0)
               $nodo->setcat_id(intval($_GET['cat_id']));
             else
 ...
 <div class="table-responsive " style="width: 100%;  ;border-color: #ffcc66;"  >
     <table id="myTable" class="table table-hover "  style=" border-color: #ffcc66;table-layout: fixed;"  >
        <thead>
  <tr class="header">
  
      <th class="col-1 col-1" style="  text-align: left;"><i class="fas fa-check"></i></th>
        <th style="<?echo $display[0]; ?>width: 1%;; text-align: center;"  ><i class="fas fa-link small"></i></th> 
              <th class="col-3 col-3" style="<?echo ""//$display[4]; ?> text-align: center;"  >  PRECIO</th> 
       <th class="col-8 col-md-8" style=" <?echo $display[1]; ?>">NOMBRE</th>
 ...
 //   $json = $json.", 'nombre_especialidad':'".$nodo->arrproducto[$i]['nombre_especialidad']."' ";
	...
	?>
       ...
       <tr class="" style="" >
      <td class="col-1 col-md-1" style="  text-align: left;"> <? 
      if(intval($_GET['origen']) == 4){ //seleccion ?>
    <button id="btn1" onclick="sel2(<?echo $json; ?>,'3');" class="btn btn-info btn-sm" title="SELECCIONAR REGISTROS1"  data-dismiss="modal"  onfocusout="" data-dismiss="modal" ><i class="far fa-hand-point-left"></i></button> 
     <? }else if(intval($_GET['origen']) == 6){   ?>
    <button id="btn1" onclick="selproducto(<?echo $json; ?>);" class="btn btn-success btn-sm small" title="SELECCIONAR REGISTROS2"  style="font-size: x-small;"  ><i class="fas fa-check"></i></button>
  
    <? }else{  ?>
   <button id="btn1" onclick="sel(<?echo $json; ?>,'3');" class="btn btn-info btn-sm" title="SELECCIONAR REGISTROS3"  data-dismiss="modal"  onfocusout="" data-dismiss="modal" ><i class="far fa-hand-point-left"></i></button>
    <? }  ?>   
        ...
        <td class="col-3 col-md-3" style="<?echo ""//$display[4]; ?>; text-align: center;" ><? echo $nodo->arrproducto[$i]['prod_precio1']; ?></td>
<!--1--><td class="col-8 col-md-8" style="font-weight: bold; <?echo $display[1]; ?>"><? echo strtoupper($nodo->arrproducto[$i]['prod_nombre']); ?></td>
 <!--2 -->  

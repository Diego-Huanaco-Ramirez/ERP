...
//require_once('modalparametro.php');
...
//$myJSON = $_GET['myJSON1']; 
$myJSON2 = json_decode($_GET['myJSON2']);
 


$lentes = $_GET['lentes'];
$rcDisPup = $_GET['rcDisPup'];
$rlDisPup = $_GET['rlDisPup'];
$nv =  $_GET['nv'];
...
//$id_agenda = 111;
$nodo = new paciente();
$nodo->setid_agenda(intval($id_agenda));
...
if($nodo->arrobtPaciente[0]['fecha_nacimiento'] != '0000-00-00'){
    $fecha = cambiaf_a_normal($nodo->arrobtPaciente[0]['fecha_nacimiento']);
   $edad = calculaedad($fecha)." AÑOS ".calculaedadmes($fecha)." MESES";
...
window.onafterprint = function(e){
        ...
        if(window.onafterprint !== 'function window.onbeforeprint()'){
          console.log("imprimio");;
          setTimeout(function(){var ww = window.open(window.location, '_self'); ww.close(); }, 2000);
           
            console.log(window.onafterprint.length);
...
<body  onload="print_receta(<? echo $id_agenda;?>);//setTimeout(function(){var ww = window.open(window.location, '_self'); ww.close(); }, 7000);"  >
   ...
   <input type="text" id="txt_id_agenda" class="form-control"   placeholder=""  value="<? echo intval($_GET['id_agenda']);?>" hidden > 
   <table class="table" >
  ...
  <tr class="header">
    ...
    <th >RUT</th>
     <th >Telefono</th>
     <th > <button type="button" class="btn btn-info" onmousemove="cerrar()" onclick="cerrar()" title="" ><i class="fas fa-toggle-off"></i></button>
     </th>
        ...
        <tbody>
            <tr>  
                <td style="font-size: small;font-weight:  bold;"><? echo $nombre;?></td>
                 
                <td style="font-size: small;"><? echo $edad;?></td>
                <td style="font-size: small;"><? echo $nodo->arrobtPaciente[0]['identificador'];?></td>
                   <td style="font-size: small;"><? echo $nodo->arrobtPaciente[0]['fono_paciente'];?></td>
   ...
   <div class="row align-self-center col-lg-12 col-12 sinmargen" style="height: 100%;" >
                <iframe id="marco_print" class="col-md-12 col-lg-12 col-sm-12 col-xs-12" src=""  onclick="" width="100%"  height="100%"  ></iframe>
            </div>
